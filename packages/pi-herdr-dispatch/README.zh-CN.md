...
预留是持久的。恢复到**同一个** Origin Session 时，它会先解析已存储的目标身份并执行一次有界 catch-up 读取，再安装目标专属订阅；缺失目标会变成 `target-lost`，并继续允许手动结算，而不是禁用 Adapter。监控永不转移给其他会话。排队的净化结果通过 `nextTurn` 送达，不会触发模型回合。
