...
一个处于分阶段开发中的 Pi 扩展，用于把经过显式确认的工作安全地派发（dispatch）给本地 Herdr 工作区中**已经存在**的 coding Agent。

> **状态：** Phase 6 实现与一次性拓扑验收已完成。本包可用于经过审阅的本地开发使用，但仍为 `private`、版本 `0.0.0-development`；是否发布需要用户单独决定。
...
- `/hd-new`（`/herdr-dispatch`）—— 创建、预览并确认一个手动派发提案。
...
- `/hd-resolve [id或前缀]`（`/herdr-dispatch-resolve`）—— 在查看证据并确认后，选择并手动（或紧急）结算一个 dispatch。
...
每个派发提案都会预览完整的出站字节并要求 TUI 确认。编辑会产生一个新的不可变提案并要求再次预览。非 TUI 模式可以列出和检查，但不能预留、发送、回复、取消、结算或监控。
...
- 精确的 TUI 确认、不可变的 payload 哈希、当前工作区范围、终端身份、关闭/移动观测、投递回显校验；
...
本扩展只在编辑器下方添加一个紧凑 widget，从不替换 Pi 的 footer。`/hd-manager`（长命令 `/herdr-dispatches`；快捷键 `alt+h`，仅 TUI）打开 Dispatch Manager：一个当前工作区、attention 优先的列表，本 Origin 最近已结算的记录默认折叠。Dispatch ID 是内部关联细节，只出现在显式的技术细节里。Manager 每次渲染都重读 Registry、刷新相对时间，输出读取只以显式的一次性有界 tail 进行（`r` 50 行、`R` 200 行，带时间戳并标注为不可信）。回复、取消和结算的选择仍然经过既有的预览、资格重验和确认闸门。
