...
- Every state-changing action (send, reply, cancel, resolve) is TUI-only, previews its exact effect, and passes a confirmation gate. Never add a code path that bypasses, weakens, or auto-approves these gates. Read-only surfaces must stay read-only.
