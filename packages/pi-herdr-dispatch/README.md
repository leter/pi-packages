...
> **⚠️ Core path verified live; full acceptance still partial.** A clean live run (2026-07-17) confirmed the load-bearing behavior: an armed settlement fires exactly one wake turn, the result reaches the model, the exactly-once delivery claim completes and does not re-fire, and nothing runs away — the feared ghost-wake loop did not occur ([details](./docs/ACCEPTANCE-RESULTS.md)). Still unverified end-to-end: multi-hop depth-limit termination, `/hd-auto off` mid-flight, and resume visibility. Until those remaining [L14](./docs/L14-auto-run-runbook.md) cases are run, treat Auto Run as experimental for real unattended work. It is off by default.
