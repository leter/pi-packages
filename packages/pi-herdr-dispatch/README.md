...
> **Verified live (2026-07-17): Auto Run passes L14.** A clean run against a real Pi + codex confirmed the full acceptance: an armed settlement fires exactly one wake turn, the result reaches the model, the exactly-once delivery claim completes and never re-fires (the feared ghost-wake loop does not occur), a two-hop chain provably terminates at the depth limit, and settle-then-arm, `/hd-auto off`, and resume visibility all behave correctly ([evidence](./docs/ACCEPTANCE-RESULTS.md)). It is off by default; arm it per session with `/hd-auto on`.
