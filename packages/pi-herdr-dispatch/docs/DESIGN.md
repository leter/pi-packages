...
`pi-herdr-dispatch` is a global Pi package for automatically dispatching work through a typed, Registry-backed path to coding agents in the Origin Pi's current workspace on one local Herdr server. It may inspect Agent metadata, dispatch work to an idle-like Existing Agent, perform one user-initiated Agent Launch before dispatch, or create bounded missing read-only-role capacity while Auto Run is armed. It cannot create workspaces and creates a Task Worktree only as an explicit user-selected Agent Launch step ([ADR 0015](./adr/0015-task-worktree-isolation.md)).
...
- Agent Launch is current-workspace and no-focus. `/hd-create` remains user-only; non-mutating launches use the Origin cwd and write launches may use the Origin cwd or first create a user-selected Task Worktree. The narrow model-callable exception is one reuse-first `non-mutating` Role launch in the Origin cwd while Auto Run is armed and Launch Budget remains. It never creates a worktree, write-role pane, or workspace. Created Agents and Task Worktrees are retained after failure and settlement ([ADR 0013](./adr/0013-user-initiated-agent-launch.md), [ADR 0015](./adr/0015-task-worktree-isolation.md), [ADR 0018](./adr/0018-model-initiated-readonly-launch.md)).
...
3. **Dispatch Registry** — global SQLite storage in WAL mode, with transactions and unique constraints for target occupancy and worktree write leases. Schema version 3 removes the obsolete Automation Grant table; version 4 adds `result_seen_at`; version 5 adds Auto Run session state and dispatch depth; version 6 adds durable Board Tasks plus Run Quota; version 7 adds task Role, Workflow, stage/rework state, parked reasons, stage feedback, and result verdicts; version 8 adds Launch Budget and its used count to armed sessions.
...
On Herdr 0.7.4, `codex` and `opencode` Agent Launch readiness accepts screen detection because their integrations do not report `agent_session`; without an exact positive signal, the resulting provenance stays **screen-detected (best effort)**.
...
The extension's own typed Herdr socket adapter does not pass through this shell gate. Its Agent Launch methods are reachable only from `/hd-create` and the bounded `herdr_agent_launch_readonly` tool; they bind the captured workspace and Origin pane, request no focus, validate returned workspace/cwd/terminal identity, and accept only the fixed supported executable catalog.
...
Wake turns stay thin. Their fixed English preamble carries the queued count, remaining Run Quota, and remaining Launch Budget, then tells the model to register the result, advance the board, and dispatch the next suitable queued task. Investigation that deserves a full task becomes a follow-up dispatch or a new draft; the extension never selects or dispatches an Agent itself.

## Roles and Workflows

ADR 0017 adds an optional team catalog at `~/.config/pi-herdr-dispatch/team.json`. Missing means the seven built-in Roles and three built-in Workflows. A present file overrides Role or Workflow entries by key; each override replaces that entry wholesale. ADR 0021 makes unknown fields file-only extension data that parsers ignore and writers preserve. Bad recognized fields, missing stage/escalation Role references, non-increasing escalation thresholds, and malformed JSON still fail closed for Role/Workflow Board Tasks. Plain tasks remain dispatchable. The runtime emits one warning for an invalid catalog.
...
- Board Tasks, approval FIFO position, bound dispatch, return feedback, Run Quota, and Launch Budget.
...
- `/hd-settings` (`/herdr-settings`) — user-only Settings panel; `alt+s` opens the same TUI. V1 exposes four stepped runtime defaults and seven Role Agent cycles, persists successful changes, and applies them without reload. It has no free-text input and no model-tool equivalent.
...
- `/hd-auto [on [N]|off]` (`/herdr-dispatch-auto`) — reports Auto Run state and depth limit plus remaining Run Quota and Launch Budget while armed; arming resets quota to N or `defaultRunQuota` and resets Launch Budget to `defaultLaunchBudget`. Disabling never manipulates Pi's message queue.
...
ADR 0021 introduces the first extension-owned writes to `~/.config/pi-herdr-dispatch/config.json` and `team.json`. The panel mirrors the Manager's pure view-model plus themed-component split, rounded frame, 96-column cap, display-width handling, and `ui-copy.ts` ownership of Simplified Chinese copy. `↑`/`↓` selects; `←`/`→` steps or cycles; `Esc` closes.

The `运行设置` group manages `defaultRunQuota` (1–50), `defaultLaunchBudget` (0–10), `maxAutoRunDepth` (1–20), and `defaultDeadlineMinutes` (the loaded minimum/maximum, step 5). The `角色模型` group manages the seven built-in Role Agent defaults in fixed launch-catalog order. All other configuration and team data remains file-only.

Each save reads the current file, preserves every unmanaged and unknown field, merges only the selected value, and validates the complete merged object through `parseDispatchConfig` or `parseTeamConfig` before any write. It then creates the parent directory if needed, writes a temporary sibling, and renames it over the destination. Validation or I/O failure leaves the destination untouched; the component re-reads the runtime ports to roll back its optimistic row and shows one failure line. A Role Agent save materializes the complete effective `label`/`mode`/`brief`/`agent` object because Role overrides replace wholesale. Successful config changes update the shared live config object used by Dispatch Application defaults; successful Role changes publish the reparsed catalog through `registry.setTeamConfigState`.

Settings is an explicit user TUI authority only. Read-only surfaces remain read-only, and no model tool gains settings access. Unknown fields are tolerated solely so newer or third-party file data survives this panel; recognized fields keep their existing fail-closed validation.

Plain `review` uses `◆` with the accent color. `▲`/warning remains reserved for actual attention, including parked review reasons `评审未过` and `评审未给结论`. The Manager keybar derives its Enter hint from the focused row: Task Board submit rows show only `enter 提交`; other rows show only `enter 详情`.
...
`defaultLaunchBudget` is the only configuration range whose minimum is zero; valid values are 0–10.
...
Unknown fields are ignored by the typed runtime view and preserved by Settings writes. Removed inspection-bound keys remain explicitly invalid. Every recognized field still passes its type, range, and cross-field checks before loading or persistence.
...
- [ADR 0021: Settings panel with persistent config and role-agent editing](./adr/0021-settings-panel.md)
