...
- **L14** live: armed session runs a two-hop dispatch chain unattended (settle → wake → follow-up dispatch → settle), including the exactly-once behavior of a `followUp`+`triggerTurn` wake against a real Pi; depth limit halts the chain into the queued path, `/hd-auto off` plus Esc stops cleanly, widget/Manager/resume visibility verified. **Status: not yet run.**

### Acceptance criteria

- U18 and U19 pass and existing suites stay green (including L9's assertion recast as "no result starts a model turn **unless Auto Run is armed and within budget**"); **I21 and L14 remain outstanding and gate final acceptance** — in particular the exactly-once behavior of a triggered wake is unverified until L14;
