...
- **U18** Auto Run wake decision: armed/disarmed, outcome coverage (all four Final Outcomes), depth attribution (per-result depth + 1), depth exhaustion, `wakeOnSettle` downgrade, preamble content and budget arithmetic, config validation;
- **U19** (added after review) `AutoRunCoordinator` orchestration at its seam: strict one-at-a-time waking, at-most-one-in-flight, wake-marker release on throw/`already-delivered`, settle-before-arm gating, and the Registry access guard;
- **I21** settlement wake end-to-end: idle triggers a turn, a burst wakes strictly one result per turn (never concurrent turns), off holds the rest, resume restores the armed switch and notifies, depth-exhausted settlement queues quietly with one notification, foreign emergency settlement wakes through the Origin path, unseen state survives auto delivery, non-TUI and foreign sessions never wake. **Status: not implemented — `DispatchRuntime.start` needs a live Herdr socket; these behaviors are covered at the coordinator seam (U18/U19) and end-to-end by L14.**
- **L14** live: armed session runs a two-hop dispatch chain unattended (settle → wake → follow-up dispatch → settle), including the exactly-once behavior of a `followUp`+`triggerTurn` wake against a real Pi; depth limit halts the chain into the queued path, `/hd-auto off` plus Esc stops cleanly, widget/Manager/resume visibility verified. **Status: PASS (2026-07-17).** Verified live against a real Pi + codex: exactly-once wake with no ghost-wake re-fire, two-hop chain terminating at the depth limit (depth-1 wake-turn dispatch recorded, its settlement queued quietly), settle-then-arm and `/hd-auto off` suppressing ignition, and arm/disarm/resume `⚡自动` visibility (ACCEPTANCE-RESULTS.md). The narrow "result held mid-turn then dropped after off" timing is covered by the coordinator unit tests.

### Acceptance criteria

- U18 and U19 pass and existing suites stay green (including L9's assertion recast as "no result starts a model turn **unless Auto Run is armed and within budget**"); **L14 passed live on 2026-07-17** — ghost-wake refuted, depth-limit termination, off, resume, and (after a fix driving the hold off `ctx.isIdle()` plus a serialized delivery queue) concurrent-burst serialization all verified against a real Pi + codex;
