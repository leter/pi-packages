...
- **L14** live: armed session runs a two-hop dispatch chain unattended (settle → wake → follow-up dispatch → settle), including the exactly-once behavior of a `followUp`+`triggerTurn` wake against a real Pi; depth limit halts the chain into the queued path, `/hd-auto off` plus Esc stops cleanly, widget/Manager/resume visibility verified. **Status: partial (2026-07-17).** The core single-hop wake path — armed settlement → exactly one wake turn → preamble+result in model context → exactly-once claim completes → no ghost-wake re-fire — passed live (ACCEPTANCE-RESULTS.md). Depth-limit termination, off-mid-flight, and resume visibility are not yet run.

### Acceptance criteria

- U18 and U19 pass and existing suites stay green (including L9's assertion recast as "no result starts a model turn **unless Auto Run is armed and within budget**"); the core wake path passed live on 2026-07-17 (ghost-wake refuted); **the remaining L14 cases — depth-limit termination, off-mid-flight, resume — still gate final acceptance**;
